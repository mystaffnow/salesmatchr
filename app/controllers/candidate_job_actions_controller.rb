class CandidateJobActionsController < ApplicationController
  ...
  def candidate_job_saved
    ...
    tracker.track('candidate-'+@candidate_job_action.candidate.email, 'viewed saved')
  end
  def candidate_job_viewed
    ...
    tracker = Mixpanel::Tracker.new(ENV["NT_MIXPANEL_TOKEN"])
    tracker.track('candidate-'+@candidate_job_action.candidate.email, 'viewed recently viewed jobs')
  end
